...
> **状態**: ✅ **完了**。9 モデル実験済み。Swift セマンティクス系 FP **-76%**、TP 0 据置。`swift-audit-rag` という別プロジェクトとして切り出した
...
| **B** RAG | 中 | カバー範囲広 | ✅ 完了 — **36→31 件 (-14%)、Swift 系 FP -76%、TP 0 据置** |
...
B (RAG) の答え合わせは下節。C (LoRA) の答え合わせは別記事に持ち越し。

## B の答え合わせ — Swift セマンティクス系 FP -76%

「RAG で Swift Book / Evolution を retrieval すれば、自分が思いつかなかった誤読パターンにも効く」という仮説 (B)。`swift-audit-rag` プロジェクトとして実装した: llama-index + bge-large (Ollama ローカル) で Swift Book 全章と Swift Evolution proposals をベクトル DB 化し、M2DX-Core のコードから `&+`・tuple 宣言・`Int32(clamping:)` などの構文を正規表現で検出 → 上位 3 chunk を取得してプロンプト先頭に差し込む構成。

v1 と同じ 9 モデル、同じコード、同じ audit プロンプト + RAG 注入ブロック先頭付き。
...
\*\* フォールトトレラント版スクリプトでタイムアウト後も継続、結果ファイルは FAILED  
...
**`&+` / `&-`** 系は完全に消えた。Swift Book の該当 chunk が retrieval で引かれ、モデルが正式な仕様を参照した結果と見られる。Swift セマンティクス系 FP の推定総数: v1 の約 17 件 → v3 の約 4 件（**-76%**）。仮説「70% 以上削減」は **達成**。
...
- RAG はプロンプト内のコードだけを見る。上流ライブラリ (`MIDI2Kit` の `maxBufferSize`)・iOS の sandbox・ファイルシステム制約はプロンプト外にあるので今回も参照不可

**H3 △ — A より広いカバレッジ、ただし総件数抑制は A の勝ち**

A (cheat sheet) は「この 5 系統を flag するな」という手書き抑制で総件数を 9 件に圧縮。B は公式仕様書全体を検索するので `&+` 系のような *仕様に明確に載っているもの* は A より確実に効く。ただし「flag するな」とは言わないので、モデルが "仕様に基づいた自信" を持ってより多く報告する副作用があり、総件数は v2 より増えた。

### 注目 finding: Q16/Q24 スケールミスマッチ

gemma4:31b と qwen3.6:35b が独立して同じ箇所を critical として報告:

```
critical: Q16/Q24 fixed-point scale mismatch in gain calculation
"exp2LookupQ24 に渡す値が DX7Envelope 出力 (Q16) と定数 (Q24) の
 混在で、massive な scale error が発生する"
```

コードを精査すると: DX7Envelope の出力は確かに Q16 スケール、`exp2LookupQ24` の定数は Q24 — **両モデルの観察は正確**。ただし実際の音声出力は正常に動作しており、コメントの精度記述のずれ（スケールの明記漏れ）が原因と判断。本実験は RAG 有効性の検証なので修正は行わない。

v1 でも v2 でも一切出なかった種類の指摘が RAG 注入で出てきた。2 モデルが独立して同じ箇所を指摘したことは、単なるハルシネーションではなく retrieval コンテキストによる一貫した推論の結果で、H3 の部分確認として記録に値する。

### 結論 (B について)

仮説 H1 ✅ Swift セマンティクス系 FP -76% → **達成**  
仮説 H2 ✅ 依存ライブラリ / 行番号の問題は残存 → **確認**  
仮説 H3 △ A より広いカバレッジ → **部分確認**（`&+` 系は完全消滅、DX7 ドメインには届かず）  
**TP は依然 0**

RAG は「Swift を知らない」という問題は実用レベルで解決できる。ただし「バグを見つける能力がない」は別軸の問題で、A も B も届かなかった。次の打ち手: C (LoRA でモデル本体に Swift 知識を刻む) か、agentic harness (grep/Read を LLM に持たせて依存ライブラリを参照させる)。
...
- **Swift カンペを pre-amble に貼った A 案**: バグ監査は **36 件 → 9 件 (-75%)** に減ったが、**TP は依然 0**。ノイズは減らせるが、未発見のバグを掘り出す力にはならない
- **RAG (B 案) の実験**: Swift Book + Evolution をベクトル DB 化してプロンプトに注入 → Swift セマンティクス系 FP **-76%** (推定 17 件→約 4 件)。`&+`/`&-` 誤読は完全消滅。ただし **TP は依然 0**、行番号捏造・依存ライブラリ未参照は残存。「Swift を知らない」は解けたが「バグを見つけられない」は別問題
