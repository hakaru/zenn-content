...
これは別にTiDBのせいじゃないし　Chroma、Pineconeでも一発のSQLでひけないだけで「ANN で取ってから加工」すれば同じような結果になりそう。（今度やってみる）

結局TiDBだと単一クエリに書けるよ！ってだけの検証になってしまう。。。
---
...
- **プロトタイプ・ローカル開発** → **Chroma**　好印象、ローカルで動くし性能悪くない。もう少し掘ってみたい。
- **チーム共有・スケールアウト** → **TiDB or Pinecone**　DBデカくなれば課金もしないと。。。。
- **既存アプリに SQL と統合・複雑な絞り込み** → **TiDB**（MySQL 互換、JOIN・集計がそのまま使える）

TiDB は「スコアが高くて類似した diff だけ引いてくる」「世代別・プロジェクト別のスコア集計」「フラグ管理」といった、SQL の表現力が必要な操作をベクトル検索と組み合わせて一発で書ける。今回の重み付き実験でスコア上は裏目に出たが、管理クエリとしての使い道は依然として強い。はず。もうちょっとTiDBに有利なシチュエーションを考えてみよう。
