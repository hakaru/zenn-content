...
- **Frontier cloud model + agentic harness**（GPT-5 / Codex / Claude Opus + Claude Code / Codex CLI / aider）
- ローカルでやりたければ **「中型ローカル LLM + grep/read ツールを与える agent」** の構成（aider や cline）

の二択が現状の最適解。 **ローカル 70B にツールを持たせた方が、ローカル 405B 単発呼び出しより精度が出る** 、という最近の感触はこの流れ。
