...
人を雇うほどの規模ではないし、有料のCloud LLMにOSSのコード全部投げるのも気が引ける。手元には **Ryzen + 64GB RAM + RTX 4090** のマシンがあって、ollamaを入れてはあったので、「ローカルLLMでバグ監査・セキュリティ監査をやってみよう」という素朴な発想だった。
...
ボロクソに書いたが、ローカルLLMが無価値というわけではない。実際、別作業でこんなことには使えていた:

- **DX7プリセット（KEYS/BASS/BRASSなど）の生成**: 「明るいE.PIANOのアルゴリズム/EG/レベル設計を提案して」のような **創造性が許される領域**。私の手元では32音色のオリジナルプリセットライブラリを LLM に下書きしてもらった
- **API コードの簡単な変換**: 既存の素直なパターンの変形 (`@Published` → `@Observable` など)
- **エラーメッセージから当たりをつける**: スタックトレース貼って「どこ見ればいい？」

つまり **「正解が一意に決まらず、創造性が報酬になる」「部分的に正しい提案でも価値がある」** タスクは依然強い。コード監査は逆に **「false positive が一つでも混ざると全体の信頼が崩れる」** タスクなので相性が悪い。

## 補足: 大手 cloud LLM ならどうか

これは未検証だが、感触として:

- **GPT-5 / Codex / Claude 4.x クラス**: コードベースを agentic に探索できる（grep/Read を投げ返す）ので、「上流の `maxBufferSize` を確認した上で判断」が可能。ローカル LLM の最大の弱点（エコシステム全体を見られない）が緩和されるはず
- ただし OSS 全体を投げるなら **データの取り扱いポリシー**を確認する必要があり、自分のプロジェクトの公開度合いで判断

近いうちに OpenAI o3 や Codex に同じプロンプトを投げて比較してみたい。
...
- 一方、**プリセット生成のような創造性タスク** ではちゃんと役に立つ

ローカル LLM は使い所を選べば武器にはなる。「コードの問題を勝手に見つけて issue 起票してくれる便利アシスタント」を期待してたなら、**今はまだ早い**。
