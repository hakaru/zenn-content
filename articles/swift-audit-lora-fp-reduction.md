---
title: "LoRA で Swift コード監査の誤検知を93%削減した話"
emoji: "🦾"
type: "tech"
...
ローカル LLM で Swift コードの静的解析を試みたら、57件の「発見」のうち真の陽性がゼロだった。そこで LoRA ファインチューニングで Swift の意味論をモデルの重みに埋め込んだところ、誤検知を93%削減できた。その過程で得た「データ量より密度」という教訓をまとめる。

## 背景：誤検知だらけの監査

M3 Ultra (96GB) + Ollama で10種類のモデルを使って Swift の音楽シンセサイザーエンジン（DX7エミュレーター）を監査した。結果は57件の「発見」、真の陽性は0件。
...
実際の監査で出た誤検知19件を DPO ペアに変換：`prompt`（コードと質問）、`chosen`（正しい「バグなし」の説明）、`rejected`（モデルが実際に出した誤検知）。

:::message
mlx-lm 0.31.3 に `mlx_lm.dpo` は存在しない。SFT 形式（`chosen` をアシスタントターンとして使用）に変換して代替した。
:::
...
これはベースモデル（Qwen2.5-Coder）が事前学習で「clamping = 値を切り捨てて精度を失う操作」という強い prior を形成していることを示す。73件の SFT では上書きできない。本格的に直すには：
...
**ベースモデルの strong prior は SFT 少数サンプルでは覆せない**
8サンプル追加しても `Int32(clamping:)` の誤解は消えなかった。ベースモデルが事前学習で形成した強い信念を覆すには DPO か大量サンプルが必要。

**データの密度 > 量**
73件の手作業サンプルが488件のOSSコードを圧倒した。ターゲットドメインに直結しないデータはシグナルを希釈する。

**Yes/No バランスは明示的に管理する**
「バグなし」だけを学習させるとモデルは常に「バグなし」と答える。真のバグサンプルを意図的に追加し、50%前後を維持することが重要。

**mlx-lm での DPO の代替**
mlx-lm 0.31.3 は DPO をサポートしない。`chosen` をアシスタントターンとした SFT で代替できる。効果は限定的だが実装コストは低い。

**早期停止を怠らない**
小データセット（< 100件）ではわずか100イテレーションで過学習する。val loss の監視と最良チェックポイントの保存は必須。
