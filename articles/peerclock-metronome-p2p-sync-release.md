...
要するに「Appleデバイス同士を±2msで時刻同期するP2Pライブラリ」。PeerClock Metronome はその最初の実使用例 / リファレンス実装でもある。
...
- 10 分後には 30ms、気持ち悪いレベルで破綻する

つまり必要なのは、

- 全デバイスで一致する時刻軸
- その時刻軸に対してクリック時刻を整列させる仕組み
- ドリフトに応じて周期的に再同期

これは PeerClock の設計そのもの。NTP 風の 4-timestamp 交換で同期し、best-half フィルタリングで Wi-Fi ジッタを均し、定常ドリフトにはバックオフしながら再同期で追従する。アプリ側から見れば `clock.now` を呼ぶだけで、全端末で揃った `UInt64` ナノ秒が返ってくる。

## 実装の芯: synced clock 駆動のビートスケジューラ

`MetronomeEngine` は普段は `mach_absolute_time()` ベースで動くが、P2P 同期モードでは PeerClock から注入される synced clock で次のビート時刻を計算する。
...
ポイントは「同期時刻上で `(n+1) * subInterval` の瞬間を次のビートと定義する」こと。この式を各端末が独立に計算しても、`syncedNow` が±2ms で揃っているので結果もその精度で一致する。小節内位置 (`currentSubBeat`) も同期時刻から導出するので、後から接続したピアが即座に正しい位置で鳴り始める。
...
`updateConfigAt` は受信した `applyAtNs` が到来するまで既存 config で刻み続け、その瞬間に切り替える。200ms の猶予を持たせてあるので、多少遅延の大きいピアでも間に合う。

結果：UI スライダーで BPM を動かすと、次の拍から全端末が揃って新しいテンポになる。誰が送信者でも同じ。「BPM を変えて鳴らし続ける操作を、2〜5台のどの端末からでも行える」。
...
PeerClock はコマンドのセマンティクスを一切知らない。「BPM 変更」も「録音開始」も「展示物のシーン遷移」も、アプリ側が `type` 文字列で定義する。時刻合わせとトランスポートの面倒な部分だけライブラリが引き取る。
...
2台以上の iPhone を同じ Wi-Fi につないでアプリを起動するだけで、自動で相互を発見して同期が始まる。「本当に揃ってる」というのを耳で確認できるのが地味に気持ちいい。

バンド練習、ドラム教室、複数モニターへのクリック配信、リハでの仮テンポ共有など、意外と使い道がある。もともとは自分のために作ったけど、PeerClock ライブラリの示威も兼ねて App Store に出した。

PeerClock 自体は音楽専用ではなく、ローカルネットワーク上での同期基盤として汎用化している。展示物の同期、複数デバイスでの同時計測、ゲームの同時開始などにも流用できる設計。興味あれば Issue / PR 歓迎。
