...
iOS/macOS向けのMIDI 2.0 FM音源アプリ **M2DX** を開発中。App Store で配信中（無料、WWDC26 週に公開）なので試してフィードバックもらえると嬉しい。心臓部の **M2DX-Core**（DX7互換エンジン、Pure Swift）はOSSで公開している。
...
- **続編 #1（B: RAG）**: ✅ 完了 — Swift セマンティクス系 FP -76%、`&+` 誤読完全消滅、TP 0。詳細は続編記事へ
...
B (RAG) と C (LoRA) の答え合わせは、それぞれ続編記事に持ち越し。
...
- **RAG (B 案) の実験**: 続編記事に詳細あり。Swift セマンティクス系 FP **-76%**、`&+`/`&-` 誤読は完全消滅。TP は依然 0
