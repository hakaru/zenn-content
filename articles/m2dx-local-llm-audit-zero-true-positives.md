...
| 案 | コスト | 仮説効果 | 進行 |
|---|---|---|---|
| **A** cheat sheet | 低 | Swift 誤検出 -70% | ✅ 完了 — **41→9 件 (-76%)、TP 0 据置** |
...
- **続編 #2（C: LoRA）**: 57 件の false positive を訓練データに転用したフィードバックループの効き目。 ***LLM が間違えた事例で LLM を再教育するとどうなるか***、というメタ的に面白いやつ

特に C は、 **本記事で集まったデータがそのまま訓練データに化ける**ところがいちばん美味しい。失敗が再起動の燃料になるという、再帰的な気持ちよさがある。

進捗が出たら、それぞれ別記事で報告する予定。

## A の答え合わせ — 41 件 → 9 件 (-76%)、TP は依然 0
...
**1. 量的には大成功**: -76%。"`&-` を未チェック overflow と誤判定" のような **絶対的な誤検出はほぼ消えた**。9B〜70B の "素直なモデル" (hermes3, llama3.3, gemma3) は cheat sheet を読んで "no bugs found" と素直に黙る側に傾いた。
...
cheat sheet は **「ローカル LLM のノイズを減らす」** には実用的に効く。前回 41 件の山を読まされる苦行が 9 件まで減った。 *ただしそこから真陽性を生み出す力はないので、本物のバグを夜中に拾い上げてくれるアシスタントは依然として frontier + agent harness が要る*。
...
ここまで書いてくると、「もっといいハード買えば突破できるのか」という気持ちが湧いてくる。**NVIDIA DGX Spark**（128GB、2台で 256GB リンク、約 60 万円〜）か、**Mac Studio M3 Ultra 256GB**（約 130 万円〜、512GB 構成可）のどっちか。…ところがこれ、 **解決しない** という結論にしかならなかった。
...
つまり「夜中にレビューを回したい」が目的なら、**60 万円のハードを増やすより crontab で Codex CLI を回したほうが安くて確実**で、これに落ち着いた。OSS なので privacy 懸念もないし。
...
- **Swift カンペを pre-amble に貼った A 案の追加実験**: バグ監査は **41 件 → 9 件 (-76%)** に減ったが、**TP は依然 0**。ノイズは減らせるが、未発見のバグを掘り出す力にはならない
